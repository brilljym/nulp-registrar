...
@section('title', 'Accounting Dashboard - NU Lipa')
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="d-flex gap-2">
                    <span class="badge" style="background:linear-gradient(135deg,#17a2b8,#138496)">
                        {{ $pendingOnsiteRequests->count() }} Onsite Pending
                    </span>
                    <span class="badge" style="background:linear-gradient(135deg,#6f42c1,#5a32a3)">
                        {{ $pendingStudentRequests->count() }} Student Pending
                    </span>
                </div>
            </div>
                                        ...
                                        <tbody>
                                            @foreach($pendingOnsiteRequests as $request)
                                                <tr class="table-row">
                                                    ...
                                                    <td>
                                                        {{ $request->full_name }}
                                                        <br><small class="text-muted">{{ $request->course }} - {{ $request->year_level }}</small>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <strong class="text-success">
                                                            ₱{{ number_format($request->requestItems->sum(fn($item) => $item->document->price * $item->quantity), 2) }}
                                                        </strong>
                                                    </td>
                                                    <td>
                                                        <img src="{{ route('accounting.receipt.onsite', $request) }}"
                                                             alt="Payment Receipt"
                                                             class="img-thumbnail"
                                                             style="max-width: 80px; max-height: 80px; cursor: pointer;"
                                                             onclick="openReceiptModal('{{ route('accounting.receipt.onsite', $request) }}', '{{ $request->ref_code ?? $request->id }}')">
                                                    </td>
                                                    <td><small class="text-muted">{{ $request->updated_at->diffForHumans() }}</small></td>
                                                    <td>
                                                        <form method="POST" action="{{ route('accounting.approve.onsite', $request) }}" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-success btn-sm action-btn" onclick="return confirm('Approve this payment?')">
                                                                <i class="bi bi-check-circle"></i> Approve
                                                            </button>
                                                        </form>
                                                        <form method="POST" action="{{ route('accounting.reject.onsite', $request) }}" class="d-inline">
                                                            @csrf
                                            ...
                                            @foreach($pendingStudentRequests as $request)
                                                <tr class="table-row">
                                                    ...
                                                    <td>
                                                        ...
                                                        <br><small class="text-muted">{{ $request->student->student_id }}</small>
                                                    </td>
                                                    <td>
                                                        @foreach($request->requestItems as $item)
                                                            <div>• {{ $item->document->type_document }} (x{{ $item->quantity }})</div>
                                                    ...
                                                    <td>
                                                        <img src="{{ route('accounting.receipt.student', $request) }}"
                                                             alt="Payment Receipt"
                                                             class="img-thumbnail"
                                                             style="max-width: 80px; max-height: 80px; cursor: pointer;"
                                                             onclick="openReceiptModal('{{ route('accounting.receipt.student', $request) }}', '{{ $request->reference_no }}')">
                                                    </td>
                                                    <td><small class="text-muted">{{ $request->updated_at->diffForHumans() }}</small></td>
                                                    <td>
                                                        <form method="POST" action="{{ route('accounting.approve.student', $request) }}" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-success btn-sm action-btn" onclick="return confirm('Approve this payment?')">
                                                                <i class="bi bi-check-circle"></i> Approve
                                                            </button>
                                                        </form>
                                                        <form method="POST" action="{{ route('accounting.reject.student', $request) }}" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-danger btn-sm action-btn" onclick="return confirm('Reject this payment?')">
                                                                <i class="bi bi-x-circle"></i> Reject
                                                            </button>
