...
@section('title', 'QR Code Management - NU Lipa')
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="d-flex gap-2">
                    <span class="badge" style="background:linear-gradient(135deg,#17a2b8,#138496)">
                        {{ $pendingOnsiteRequests->count() }} Onsite Requests
                    </span>
                    <span class="badge" style="background:linear-gradient(135deg,#6f42c1,#5a32a3)">
                        {{ $pendingStudentRequests->count() }} Student Requests
                    </span>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="row g-4 align-items-center">
                        <div class="col-md-5 text-center">
                            <img src="{{ $defaultQrUrl }}" alt="Default Payment QR" class="default-qr-preview mb-2" id="defaultQrPreviewImage">
                            <div class="text-muted small">Current default QR code</div>
                        </div>
                        <div class="col-md-7">
                            <form method="POST" action="{{ route('accounting.qr.default.upload') }}" enctype="multipart/form-data">
                                @csrf
            ...
            <div class="card shadow mb-4">
                    ...
                    <ul class="nav nav-tabs" id="qrTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="onsite-tab" data-bs-toggle="tab" data-bs-target="#onsite-pane" type="button" role="tab">
                                ...
                                <span class="tab-count onsite">{{ $pendingOnsiteRequests->count() }}</span>
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="student-tab" data-bs-toggle="tab" data-bs-target="#student-pane" type="button" role="tab">
                                ...
                                <span class="tab-count student">{{ $pendingStudentRequests->count() }}</span>
                            </button>
                        </li>
                    </ul>
                </div>
                ...
                <div class="tab-content" id="qrTabContent">
                    ...
                    <div class="tab-pane fade show active" id="onsite-pane" role="tabpanel">
                        <div class="card-body">
                            @if($pendingOnsiteRequests->count() > 0)
                                <div class="table-responsive">
                                    <table class="table table-hover align-middle qr-table">
                                        ...
                                        <tbody>
                                            @foreach($pendingOnsiteRequests as $request)
                                                <tr class="qr-table-row">
                                                    <td>
                                                        <strong>{{ $request->ref_code ?? $request->id }}</strong>
                                                        <br><small class="text-muted">{{ $request->queue_number ?? '' }}</small>
                                                    </td>
                                                    <td>
                                                        {{ $request->full_name }}
                                                    </td>
                                                    <td>
                                                        <small class="text-muted">{{ $request->course }} - {{ $request->year_level }}</small>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        @if($request->qr_code_path)
                                                            <img src="{{ $request->qr_code_path }}"
                                                                 alt="QR Code"
                                                                 class="qr-thumbnail"
                                                                 onclick="openQRPreview('{{ $request->qr_code_path }}', '{{ $request->ref_code ?? $request->id }}')">
                                                        @else
                                                    ...
                                                    <td>
                                                        @if($request->qr_code_path)
                                                            <button type="button" class="btn btn-info btn-sm upload-btn" onclick="openQRUploadModal('onsite', {{ $request->id }}, '{{ $request->ref_code ?? $request->id }}')">
                                                                <i class="bi bi-pencil-square me-1"></i>Replace
                                                            </button>
                                                        @else
                                                            <button type="button" class="btn btn-primary btn-sm upload-btn" onclick="openQRUploadModal('onsite', {{ $request->id }}, '{{ $request->ref_code ?? $request->id }}')">
                    ...
                    <div class="tab-pane fade" id="student-pane" role="tabpanel">
                        <div class="card-body">
                            @if($pendingStudentRequests->count() > 0)
                                <div class="table-responsive">
                                    <table class="table table-hover align-middle qr-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach($pendingStudentRequests as $request)
                                                <tr class="qr-table-row">
                                                    <td>
                                                        <strong>{{ $request->reference_no ?? 'N/A' }}</strong>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        @if($request->qr_code_path)
                                                            <img src="{{ $request->qr_code_path }}"
                                                                 alt="QR Code"
                                                                 class="qr-thumbnail"
                                                                 onclick="openQRPreview('{{ $request->qr_code_path }}', '{{ $request->reference_no }}')">
                                                        @else
                                                            <span class="text-muted">No QR Code</span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if($request->qr_code_path)
                                                            <button type="button" class="btn btn-info btn-sm upload-btn" onclick="openQRUploadModal('student', {{ $request->id }}, '{{ $request->reference_no }}')">
                                                                <i class="bi bi-pencil-square me-1"></i>Replace
                                                            </button>
                                                        @else
                                                            <button type="button" class="btn btn-primary btn-sm upload-btn" onclick="openQRUploadModal('student', {{ $request->id }}, '{{ $request->reference_no }}')">
                                                                <i class="bi bi-cloud-upload me-1"></i>Upload
                                                            </button>
                                                        @endif
                                                    </td>
