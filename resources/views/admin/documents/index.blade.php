    ...
    <div class="row mb-4">
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="document-card position-relative">
                ...
                <div class="stats-number primary-stat">{{ $documents->total() }}</div>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="document-card position-relative">
                ...
                <div class="stats-number success-stat">₱{{ number_format(\App\Models\Document::avg('price') ?? 0, 2) }}</div>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="document-card position-relative">
                ...
                <div class="stats-number info-stat">₱{{ number_format(\App\Models\Document::max('price') ?? 0, 2) }}</div>
        ...
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="document-card position-relative">
                ...
                <div class="stats-number warning-stat">₱{{ number_format(\App\Models\Document::min('price') ?? 0, 2) }}</div>
                ...
                <div class="table-responsive">
                    <table class="table table-documents" id="documentsTable">
                        ...
                        <tbody>
                            ...
                            <tr class="document-row" data-price="{{ $document->price }}">
                                <td><strong>{{ $document->id }}</strong></td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-file-alt me-2" style="color: #2c3192;"></i>
                                        <div>
                                            <strong>{{ $document->type_document }}</strong>
                                ...
                                <td>
                                    <span class="price-badge">₱{{ number_format($document->price, 2) }}</span>
                                </td>
                                <td>
                                    @php
                                        $requestCount = \App\Models\StudentRequestItem::where('document_id', $document->id)->count();
                                    @endphp
                                    <span class="badge bg-info">{{ number_format($requestCount) }} requests</span>
                                </td>
                                <td>
                                    @php
                                    ...
                                    @endphp
                                    <strong style="color: #28a745;">₱{{ number_format($revenue, 2) }}</strong>
                                </td>
                                <td>
                                ...
                                <td class="no-print">
                                    <div class="btn-group" role="group">
                                        <button type="button" class="btn btn-sm btn-outline-primary" 
                                                onclick="editDocument({{ $document->id }})" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-info" 
                                                onclick="viewDocumentStats({{ $document->id }})" title="View Stats">
                                            <i class="fas fa-chart-bar"></i>
                                        </button>
                                        <button type="button" class="btn btn-sm btn-outline-{{ $document->is_active ?? true ? 'warning' : 'success' }}" 
                                                onclick="toggleDocumentStatus({{ $document->id }})" 
                                                title="{{ $document->is_active ?? true ? 'Deactivate' : 'Activate' }}">
                                            <i class="fas fa-{{ $document->is_active ?? true ? 'pause' : 'play' }}"></i>
                                        </button>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="document-card">
                ...
                <div class="row">
                    <div class="col-md-3 text-center">
                        <div class="mb-3">
                            <div class="stats-number text-primary">{{ \App\Models\Document::count() }}</div>
                            <div class="stats-label">Total Document Types</div>
                        </div>
                    </div>
                    <div class="col-md-3 text-center">
                        <div class="mb-3">
                            <div class="stats-number text-success">{{ \App\Models\StudentRequestItem::distinct('document_id')->count() }}</div>
                        ...
                        <div class="mb-3">
                            @php
                                $totalRevenue = \App\Models\StudentRequestItem::join('student_requests', 'student_request_items.student_request_id', '=', 'student_requests.id')
                                              ...
                                              ->sum(\DB::raw('student_request_items.price * student_request_items.quantity'));
                            @endphp
                            <div class="stats-number text-info">₱{{ number_format($totalRevenue, 2) }}</div>
                            <div class="stats-label">Total Revenue</div>
                        </div>
                    </div>
                    <div class="col-md-3 text-center">
                        <div class="mb-3">
                            <div class="stats-number text-warning">{{ \App\Models\StudentRequestItem::sum('quantity') }}</div>
