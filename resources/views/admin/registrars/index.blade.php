            ...
            <tbody>
                ...
                <tr class="table-row">
                    <td class="text-center">
                        <span class="registrar-id">#{{ $registrar->id }}</span>
                    </td>
                    <td>
                        @if($registrar->user)
                            <span class="registrar-name">
                                {{ $registrar->user->last_name }}, {{ $registrar->user->first_name }} 
                                @if($registrar->user->middle_name)
                                    {{ $registrar->user->middle_name }}
                                @endif
                            </span>
                        @else
                    ...
                    <td class="text-primary">
                        {{ $registrar->user->school_email ?? 'N/A' }}
                    </td>
                    <td class="text-center">
                        <span class="window-badge">
                            <i class="fas fa-window-maximize"></i>
                            Window {{ $registrar->window_number }}
                        </span>
                    </td>
    ...
    <div class="d-flex justify-content-between align-items-center mt-4">
        <div class="pagination-info">
            <small class="text-muted">
                @if($registrars->total() > 0)
                    Showing {{ $registrars->firstItem() }} to {{ $registrars->lastItem() }} of {{ $registrars->total() }} registrars
                @else
        ...
        <div class="pagination-wrapper">
            {{ $registrars->links('vendor.pagination.bootstrap-5') }}
        </div>
    </div>
</div>
