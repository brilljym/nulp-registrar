...
<div class="container mt-5">
    ...
    <h2 class="mb-4">
        ...
        {{ request('search') ? 'Search Results' : 'All Document Requests' }}
    ...
    @if(request('search'))
        <div class="alert alert-info">
            Showing {{ $all->total() }} result{{ $all->total() === 1 ? '' : 's' }} for:
            <strong>"{{ request('search') }}"</strong>
            <a href="{{ route('registrar.dashboard') }}" class="btn btn-sm btn-outline-secondary ms-2">Clear</a>
        </div>
    @endif
    ...
    @if($all->isEmpty())
        <div class="alert alert-info text-center mt-4">
            No document requests found{{ request('search') ? ' matching "' . request('search') . '"' : '.' }}
        </div>
    @endif
    ...
    <div class="d-flex justify-content-between align-items-center mt-4">
        <div class="pagination-info">
            <small class="text-muted">
                Showing {{ $all->firstItem() }} to {{ $all->lastItem() }} of {{ $all->total() }} requests
            </small>
        </div>
        <div class="pagination-wrapper">
            {{ $all->appends(['search' => request('search')])->links('vendor.pagination.bootstrap-5') }}
        </div>
    </div>
</div>
