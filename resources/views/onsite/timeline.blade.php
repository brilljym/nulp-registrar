...
<body>
        ...
        <main class="main-content">
            ...
            <div class="timeline-container">
                <div class="timeline-header">
                    ...
                    <p style="margin: 0; font-size: 0.9rem; opacity: 0.9;">Requested Document: {{ $onsiteRequest->requestItems->pluck('document.type_document')->join(', ') }}</p>
                </div>
    ...
    @foreach (['success', 'error', 'info'] as $msg)
        @if (session($msg))
            <div class="alert alert-{{ $msg === 'error' ? 'danger' : ($msg === 'info' ? 'info' : 'success') }} text-center">
                {{ session($msg) }}
            </div>
        @endif
    @endforeach
                    ...
                    <div class="mb-2 d-flex justify-content-center">
                        <div class="rounded-circle d-flex align-items-center justify-content-center shadow-sm"
                             style="width: 55px; height: 55px;
                                    background-color:
                                        {{ $index < $currentIndex ? '#198754' : ($index === $currentIndex ? '#0d6efd' : '#dee2e6') }};
                                    color: white; font-size: 24px;">
                            {{ $step['icon'] }}
                        </div>
                    </div>
                    <div class="fw-semibold small
                        {{ $index < $currentIndex ? 'text-success' : ($index === $currentIndex ? 'text-primary' : 'text-muted') }}">
                        {{ $step['label'] }}
                    </div>
                </div>
            @endforeach
    ...
    {{-- Request Details Summary - Only show during payment step --}}
    {{-- Approval Waiting Section - Show for pending requests --}}
    @if ($onsiteRequest->status === 'pending')
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <br><strong>Total Quantity:</strong> {{ $onsiteRequest->requestItems->sum('quantity') }}
                                <br><br>
                                <div class="text-center">
                                    ...
                                    @endphp
                                    <img src="{{ $onsiteRequest->qr_code_path ?: asset($defaultQrPath) }}" alt="Payment QR Code" class="img-fluid" style="max-width: 200px; max-height: 200px;">
    ...
    <div class="text-center mt-5">
        ...
        @if ($onsiteRequest->current_step === 'payment')
            @if($onsiteRequest->payment_approved)
                ...
                <form method="POST" action="{{ route('onsite.reference.submit') }}" class="d-flex justify-content-center align-items-center gap-2">
                    @csrf
            ...
            @elseif($onsiteRequest->payment_receipt_path)
                ...
                <div class="text-center">
                    <p class="text-muted">Receipt submitted: {{ $onsiteRequest->updated_at->diffForHumans() }}</p>
                </div>
            @else
                ...
                <div class="row justify-content-center">
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-body">
                                <form method="POST" action="{{ route('onsite.upload.receipt', $onsiteRequest) }}" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-3">
                ...
                <div class="col-md-6">
                    <div class="card border-0 shadow-sm">
                        <div class="card-body text-center">
                            ...
                            @if ($onsiteRequest->registrar)
                                <h5 class="text-primary mb-2">
                                    {{ $onsiteRequest->registrar->first_name ?? 'Registrar' }} 
                                    {{ $onsiteRequest->registrar->last_name ?? '' }}
                                </h5>
                            @else
                                <p class="text-muted mb-0"><em>Registrar not yet assigned</em></p>
                            @endif
                            <hr class="my-2">
                            <p class="text-monospace text-secondary mb-0" style="font-size: 0.8rem;">{{ $ticketNumber }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        @elseif ($onsiteRequest->current_step === 'completed')
                ...
                <div class="col-md-8">
                    <div class="card border-success shadow-sm">
                        ...
                        <div class="card-body text-center">
                            ...
                            <hr class="my-3">
                            <p class="text-monospace text-secondary mb-2">{{ $ticketNumber }}</p>
...
{{-- Modal for Initial Info Input --}}
@if (!$onsiteRequest->full_name || !$onsiteRequest->course || !$onsiteRequest->year_level || !$onsiteRequest->department)
    <script>
...
<div class="modal fade" id="infoModal" tabindex="-1" aria-labelledby="infoModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <form method="POST" action="{{ route('onsite.update', $onsiteRequest->id) }}" class="modal-content">
            @csrf
            @method('PUT')
            ...
            <div class="modal-body row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="full_name" class="form-control" value="{{ $onsiteRequest->full_name }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Student ID (if any)</label>
                    <input type="text" name="student_id" class="form-control" value="{{ $onsiteRequest->student_id }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Course</label>
                    <input type="text" name="course" class="form-control" value="{{ $onsiteRequest->course }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Year Level</label>
                    <input type="text" name="year_level" class="form-control" value="{{ $onsiteRequest->year_level }}" required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Department</label>
                    <input type="text" name="department" class="form-control" value="{{ $onsiteRequest->department }}" required>
                </div>

                <div class="col-12">
                    <h6>Document Request</h6>
                    <div id="modal-documents-container">
                        @if($onsiteRequest->requestItems->count() > 0)
                            @foreach($onsiteRequest->requestItems as $index => $item)
                                <div class="document-item mb-3 p-3 border rounded">
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <label class="form-label">Document Type *</label>
                                            <select class="form-select document-select" name="documents[{{ $index }}][document_id]" required>
                                                <option value="" disabled>-- Select Document Type --</option>
                                                @foreach($documents ?? collect() as $document)
                                                    <option value="{{ $document->id }}" data-price="{{ $document->price }}" {{ $item->document_id == $document->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <input type="number" class="form-control quantity-input" name="documents[{{ $index }}][quantity]"
                                                   value="{{ $item->quantity }}" min="1" max="150" required>
                                        </div>
                                        <div class="col-md-2 mb-3 d-flex align-items-end">
                                            <button type="button" class="btn btn-danger remove-document" style="{{ $onsiteRequest->requestItems->count() > 1 ? '' : 'display: none;' }}">
                ...
                <input type="hidden" name="reason" value="{{ $onsiteRequest->reason }}">
                @endif
                <!-- Include document_id -->
                <input type="hidden" name="document_id" value="{{ $onsiteRequest->document_id }}">
            </div>
    ...
    const channelName = 'request-{{ $onsiteRequest->ref_code }}';
    ...
    requestChannel.bind('pusher:subscription_error', function(err) {
        ...
        setInterval(function() {
            ...
            .then(html => {
                ...
                const currentStep = '{{ $onsiteRequest->current_step }}';
                const paymentApproved = '{{ $onsiteRequest->payment_approved ? 'true' : 'false' }}';
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let modalDocumentIndex = {{ $onsiteRequest->requestItems->count() ?: 1 }};
    ...
    <script>
        ...
        function skipRedirect() {
            ...
            if (countdownEl) {
                countdownEl.innerHTML = '<span class="text-success">You can stay on this page. <a href="{{ route('login') }}">Click here to go to login</a></span>';
            }
        }
