...
<body>
    ...
    <div class="content-wrapper">
        ...
        <main class="main-content">
            <div class="confirm-container">
                ...
                @if(isset($request->queue_number) && $request->queue_number)
                    ...
                    <div class="queue-status mb-3">
                        @if(isset($isReady) && $isReady)
                            <div class="alert alert-success p-2" style="border-radius: 8px;">
                                <i class="fas fa-bell me-2"></i>
                                <strong>Please proceed to Window {{ $windowAssignment ?? 'TBD' }}</strong>
                            </div>
                        @else
                ...
                <div class="request-details">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">{{ $type === 'student' ? $request->reference_no : $request->ref_code }}</span>
                    </div>

                    @if($type === 'student')
                        <div class="detail-row">
                            <span class="detail-label">Student:</span>
                            <span class="detail-value">{{ $request->student->user->first_name }} {{ $request->student->user->last_name }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Course:</span>
                            <span class="detail-value">{{ $request->student->course }}</span>
                        </div>
                        <div class="detail-row">
                            <span class="detail-label">Year Level:</span>
                            <span class="detail-value">{{ $request->student->year_level }}</span>
                        </div>
                    @else
                        <div class="detail-row">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="status-badge status-ready">
                            ...
                            @elseif ($request->status === 'released')
                                Ready for Release  
                            @else
                                {{ $type === 'student' ? 'Ready for Release' : 'Released' }}
                            @endif
                        </span>
                    </div>
                ...
                <form action="{{ route('kiosk.confirm-pickup', ['type' => $type, 'id' => $request->id]) }}" method="POST">
                    @csrf
                    @if(isset($isReady) && $isReady)
                        @if($request->status === 'ready_for_release' || $request->status === 'released')
                ...
                <a href="{{ route('kiosk.index') }}" class="btn btn-secondary btn-back d-block">
    ...
    <script>
        let queueNumber = '{{ $request->queue_number }}';
        let isCurrentlyReady = {{ isset($isReady) && $isReady ? 'true' : 'false' }};
        ...
        queueUpdatesChannel.bind('realtime.notification', function(data) {
            ...
            if (data.data && (
                ...
                data.data.reference_code === '{{ $type === 'student' ? $request->reference_no : $request->ref_code }}'
            )) {
        ...
        function checkQueueStatus() {
            fetch(`{{ route('kiosk.queue-status') }}?queue_number=${queueNumber}`)
                ...
                Notification.requestPermission().then(function (permission) {
                    if (permission === 'granted') {
                        new Notification('NU Regis Queue', {
                            body: 'Please proceed to Window {{ $windowAssignment ?? "TBD" }}',
                            icon: '/favicon.ico'
                        });
                    }
                });
        ...
        function printReceiptAfterConfirm() {
            ...
            return new Promise((resolve, reject) => {
                ...
                fetch(`{{ route('kiosk.print-receipt', ['type' => $type, 'id' => $request->id]) }}`, {
                    method: 'POST',
                    headers: {
        ...
        function testPrinter() {
            fetch(`{{ route('kiosk.test-printer') }}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
