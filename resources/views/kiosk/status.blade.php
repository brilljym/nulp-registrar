...
<body>
        ...
        <main class="main-content">
            <div class="status-container">
                @if(isset($error))
                    ...
                    <div class="error-container">
                        ...
                        <div class="error-description">
                            The queue number <strong>{{ $queueNumber }}</strong> was not found in our system.
                        </div>
                        <a href="{{ route('kiosk.index') }}" class="btn btn-light btn-back">
                ...
                @else
                    ...
                    <!-- Status Card -->
                    <div class="status-card status-{{ $statusInfo['status'] }} {{ in_array($statusInfo['status'], ['in_queue', 'processing']) ? 'status-active' : '' }}">
                        <div class="status-icon">
                            <i class="{{ $statusInfo['statusIcon'] }}"></i>
                        </div>
                        <div class="status-title">{{ $statusInfo['statusText'] }}</div>
                        <div class="status-description">{{ $statusInfo['description'] }}</div>

                        @if($statusInfo['status'] === 'waiting')
                            <div class="position-info">
                                <div class="position-number">{{ $statusInfo['position'] }}</div>
                                <span>in queue</span>
                            </div>
                    ...
                    @if($statusInfo['status'] === 'waiting' && $statusInfo['position'] > 0)
                        <div class="progress-container">
                            <div class="progress">
                                <div class="progress-bar" style="width: {{ min(100, (1 / $statusInfo['position']) * 100) }}%"></div>
                            </div>
                    ...
                    <div class="text-center">
                        ...
                        <br>
                        <a href="{{ route('kiosk.index') }}" class="btn btn-secondary btn-back">
                            <i class="fas fa-arrow-left me-2"></i>Back to Kiosk
                        </a>
                    </div>
                @endif
    ...
    <script>
        ...
        realTimeUpdatesChannel.bind('realtime.notification', function(data) {
            ...
            if (data.data && (
                data.data.queue_number === queueNumber ||
                data.data.id === parseInt('{{ $request->id }}')
            )) {
                handleQueueUpdate(data);
            }
        });
