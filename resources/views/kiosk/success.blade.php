...
<body>
        ...
        <main class="main-content">
            <div class="success-container">
                ...
                <div class="success-message">
                    @if(isset($message))
                        <p style="color: var(--accent-color); font-weight: 600; margin-bottom: 1rem;">{{ $message }}</p>
                    @endif
                    <p>
                        <strong>Reference Code:</strong> {{ $type === 'student' ? $request->reference_no : $request->ref_code }}
                    </p>
                    @if(isset($request->queue_number) && $request->queue_number)
                        <p>
                            <strong>Queue Number:</strong> {{ $request->queue_number }}
                        </p>
                    @endif
    ...
    <script>
        ...
        @if(isset($request->queue_number) && $request->queue_number)
            console.log('Enhanced wait time script running for queue:', '{{ $request->queue_number }}');
            var queuePosition = @json($queuePosition ?? 2);
            var requestId = '{{ $type === 'student' ? $request->reference_no : $request->ref_code }}';
            var serviceType = '{{ $type === 'student' ? 'student_documents' : 'alumni_documents' }}';
