...
// Temporary route to update A004 with real data
Route::get('/update-a004-real-data', function () {
    try {
        ...
        // Find the existing A004 onsite request
        $existingA004 = \App\Models\OnsiteRequest::where('queue_number', 'A004')->first();
        if (!$existingA004) {
            return response()->json(['error' => 'A004 onsite request not found']);
        }

        // Get the real student request data (SR-20251028-0001)
        $studentRequest = \App\Models\StudentRequest::with(['student.user'])
            ->where('reference_no', 'SR-20251028-0001')
            ->first();

        if (!$studentRequest) {
            return response()->json(['error' => 'Student request SR-20251028-0001 not found']);
        }

        // Update the existing A004 record with real data
        ...
        return response()->json([
            'success' => true,
            'message' => 'Updated A004 with real student data',
            'updated' => [
                'queue_number' => $existingA004->queue_number,
                'full_name' => $existingA004->full_name,
                'course' => $existingA004->course,
                'department' => $existingA004->department,
            ]
        ]);
